...
function ProductPage() {
    ...
    const images = product.pictures.map((picture) => <img className="product__carousel--image" src={picture} onDragStart={handleDragStart} />);
        ...
        <Container className="pt-4" style={{ position: "relative" }}>
            <Row>
                ...
                <Col lg={6} className="pt-4">
                    <h1>{product.name}</h1>
                    <p>
                        <Badge bg="primary">{product.category.charAt(0).toUpperCase() + product.category.slice(1)}</Badge>
                    </p>
                    ...
                    {user && !user.isAdmin && (
                        <ButtonGroup style={{ width: "90%" }}>
                            ...
                            <Button size="lg" onClick={() => addToCart({ userId: user._id, productId: id, price: product.price, image: product.pictures[0] })}>
                                Add to cart
                            </Button>
                        </ButtonGroup>
                    )}
